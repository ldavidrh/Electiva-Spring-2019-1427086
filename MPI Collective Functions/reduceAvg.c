...
int main(int argc, char *argv[])
{
    ...
    //Calcular el promedio de los promedios calculados por cada proceso usando la funcion MPI_Reduce.
    float sumaGlobal;
    MPI_Reduce(&sub_vector_avg, &sumaGlobal, 1, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
    if (myrank == 0)
    {
        printf("El promedio calculado con los procesos es %.6f\n", sumaGlobal/worldsize);
