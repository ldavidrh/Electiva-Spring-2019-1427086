...
#define MAXVECT 10
#define MAXPROC 5
#define imprimirvector printf("Mi vista (%d) del vector\n", myrank); for (i = 0; i < MAXVECT; i++) printf("[%d: %d] ", myrank, invect[i]); printf("\n");
...
int main(int argc, char *argv[])
{
    ...
    if (myrank == 0)
    {
        for (i = 0; i < worldsize; i++)
        {
            if (myrank != i)
            {
                MPI_Send(&outvect, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
    ...
    {
        MPI_Recv(&invect, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &Stat);
